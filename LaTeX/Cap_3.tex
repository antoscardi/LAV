...
\end{aligned}
\]
where $\mathbf{R}_t^g$ is the rotation matrix that rotates axis $g$ to $t$ \cite{artva-gazebo}.
\begin{comment}
...
where \( k, i \in \mathbb{N} \) and \( k = 1, \ldots, n \).
...
the reference frame $F_i$ are denoted with $R^g_{t_i}$.
Instead the rotation matrices that describe the orientation between the receiver
frame and the multiple trasmitters are defined as $R^{t_i}_r$.
...
when projected in the receiver frame $F_r$, as in \cite{spacecraft}:
\begin{equation}
...
in the transmitter reference frame $F_t$, in the same way as Eq.\ref{eq:pt_multi}.

Secondly, we use the analytical expression in Eq.\ref{eq:G_analit}, to compute the gradient tensor 
in the transmitter frame ${}^t \mathbf{G}_i$, of the $i$-th source.
Then we transform ${}^t \mathbf{G}_i$ in ${}^r \mathbf{G}_i$, the gradient expressed in the receiver frame using Eq.\ref{eq:rotated_gradient_tensor}.
...
\mathbf{R}^{t_1}_r  \, {}^t\mathbf{G}_1({}^{t_1} \mathbf{p}) \, {\mathbf{R}^{t_1}_r}\mkern-2mu^\top + \dots + \mathbf{R}^{t_n}_r  \, {}^t\mathbf{G}_n({}^{t_n} \mathbf{p}) \, {\mathbf{R}^{t_n}_r}\mkern-2mu^\top \nonumber = \\
&= 
\mathbf{R}^{t_1}_r  \, {}^t\mathbf{G}_1({R^g_{t_1}}\mkern-4mu^\top ( \mathbf{p}_r - \mathbf{p}_{t_1})) \, {\mathbf{R}^{t_1}_r}\mkern-3mu^\top + 
\cdots + \mathbf{R}^{t_n}_r   {}^t\mathbf{G}_n({R^g_{t_1}}\mkern-3mu^\top ( \mathbf{p}_r - \mathbf{p}_{t_1}))  {\mathbf{R}^{t_n}_r}\mkern-3mu^\top
\label{eq:G_tot}
\end{align}
...
We will now detail the precise passages of how we compute in the final model.
FORME MEGLIO SE SCEGLIAMO NUMERICAL NON SO MAGARI CHIEDERE AL PROF
Since there is no effective difference between the two methods we will use the more theoretical
one (analytical) in the simulations,for clarity and simplicity.

Also, the drones in the multiple victims case should be equipped with an instrument able to receive the 
total magnetic field and compute the gradient tensor matrix like the one used in \ref{tri axial etx },
and therefore no more the ARTVA instrument, while the victims continue to be equipped with the same instruments.
se fosse stato nel caso di una singloa vittima si poteva calcolare il gardiente analiticamente usando solo 
la posizione del drone (ij assenza di rotazioni), invece nel caso multi vittime non si ha un espressione 
esplicita quindi simuliamo il caso reale usando il caso numerico avendo a disposizione appunto uno strumento
che riesce a ottenere info sul campo elettromagnetico in piu punti.


\subsection{Particle Swarm Optimization}
In the context of search and rescue (S\&R) missions, the number of available 
UAVs is typically limited (ranging from 1 to 10 \cite{5}), and the search area 
usually spans only a few thousand square meters. Therefore, it is reasonable to 
